...
export async function getPublicVacancies(filters?: { employmentType?: string; workplaceType?: string; location?: string; status?: string }) {
  ...
  let query = supabase.from("vacancies").select(publicColumns).in("status", publicVacancyStatuses).not("published_at", "is", null).or(`closes_at.is.null,closes_at.gt.${new Date().toISOString()}`).order("published_at", { ascending: false }).limit(50);
...
export async function getPublicVacancy(slug: string) {
  ...
  const supabase = await createClient();
  const { data, error } = await supabase.from("vacancies").select(publicColumns).eq("slug", slug).in("status", publicVacancyStatuses).not("published_at", "is", null).or(`closes_at.is.null,closes_at.gt.${new Date().toISOString()}`).maybeSingle();
  return error || !data ? null : (data as PublicVacancy);
}

export async function getPublicVacancySitemapEntries() {
  const supabase = await createClient();
  const { data, error } = await supabase.from("vacancies").select("slug, updated_at").in("status", publicVacancyStatuses).not("published_at", "is", null).or(`closes_at.is.null,closes_at.gt.${new Date().toISOString()}`).order("published_at", { ascending: false }).limit(1000);
  return error ? [] : data ?? [];
}
